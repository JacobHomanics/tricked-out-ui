...
namespace JacobHomanics.TrickedOutUI
{
    ...
    /// </summary>
    public class BackgroundFillFeatureComponent2 : BaseCurrentMaxComponent
    {
        [System.Serializable]
        ...
        void Update()
        {
            backgroundFillFeature.backgroundFill.fillAmount = HandleValueChange(Current, backgroundFillFeature.backgroundFill.fillAmount, ref previousValue, Max, backgroundFillFeature.keepSizeConsistent, backgroundFillFeature.delay, backgroundFillFeature.speedCurve, backgroundFillFeature.animationSpeed);
            backgroundFillFeature.backgroundFill.fillAmount = UpdateBackgroundFillAnimation(Max);
        }

        public float HandleValueChange(float newValue, float fillAmount, ref float previousValue, float max, bool keepSizeConsistent, float delay, AnimationCurve curve, float speed)
        {
            ...
            {
                StartBackgroundFillAnimation(startValue, newValue, max, delay, curve, speed, ref fillAmount);
            }
        ...
        public float UpdateBackgroundFillAnimation(float max)
        {
            if (!isAnimating)
                return 0;
            ...
            float fillAmount = Normalize(currentValue, max);
